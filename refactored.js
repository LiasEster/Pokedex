...
async function displayPokemonDetails(poke, index, pokemons) {
    const data = await fetchPokemonData(`https://pokeapi.co/api/v2/pokemon/${poke.name}`);
    ...
    document.addEventListener('keydown', function(event) {
        if (overlay.style.display === 'flex' && (event.key === 'ArrowRight' || event.key === 'ArrowLeft')) {
            const index = allPokemon.findIndex(poke => poke.name === overlay.querySelector('.pokemon-details-card h2').textContent.toLowerCase());
...
function addArrowClickEvents(index, pokemons) {
  ...
  // Update the references to the arrows
